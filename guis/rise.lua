...
local getcustomassets = {
	['oofer-vape/assets/rise/slice.png'] = 'rbxasset://risesix/slice.png',
	['oofer-vape/assets/rise/blur.png'] = 'rbxasset://risesix/blur.png',
	['oofer-vape/assets/new/blur.png'] = 'rbxassetid://14898786664',
}
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('oofer-vape/assets/new/blur.png')
...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/'..readfile('oofer-vape/profiles/commit.txt')..'/'..select(1, path:gsub('oofer-vape/', '')), true)
		end)
...
local function writeFont()
	...
	writefile('oofer-vape/assets/rise/risefont.json', httpService:JSONEncode({
		name = 'ProductSans',
		faces = {
			{style = 'normal', assetId = getcustomasset('oofer-vape/assets/rise/SF-Pro-Rounded-Light.otf'), name = 'Light', weight = 300},
			{style = 'normal', assetId = getcustomasset('oofer-vape/assets/rise/SF-Pro-Rounded-Regular.otf'), name = 'Regular', weight = 400},
			{style = 'normal', assetId = getcustomasset('oofer-vape/assets/rise/SF-Pro-Rounded-Medium.otf'), name = 'Medium', weight = 500},
			{style = 'normal', assetId = getcustomasset('oofer-vape/assets/rise/Icon-1.ttf'), name = 'Icon1', weight = 600},
			{style = 'normal', assetId = getcustomasset('oofer-vape/assets/rise/Icon-3.ttf'), name = 'Icon3', weight = 800}
		}
	}))
	return getcustomasset('oofer-vape/assets/rise/risefont.json')
end
...
do
	...
	local res = isfile('oofer-vape/profiles/color.txt') and loadJson('oofer-vape/profiles/color.txt')
	if res then
				...
				if optionsettings.Profiles then
					...
					if ind then
						if val ~= 'default' then
							...
							if isfile('oofer-vape/profiles/'..val..mainapi.Place..'.txt') and delfile then
								delfile('oofer-vape/profiles/'..val..mainapi.Place..'.txt')
							end
						end
					else
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('oofer-vape/profiles/'..game.GameId..'.gui.txt') then
		guidata = loadJson('oofer-vape/profiles/'..game.GameId..'.gui.txt')
		if not guidata then
			guidata = {Categories = {}}
	...
	if isfile('oofer-vape/profiles/'..self.Profile..self.Place..'.txt') then
		local savedata = loadJson('oofer-vape/profiles/'..self.Profile..self.Place..'.txt')
...
function mainapi:Save(newprofile)
	...
	writefile('oofer-vape/profiles/'..game.GameId..'.gui.txt', httpService:JSONEncode(guidata))
	writefile('oofer-vape/profiles/'..self.Profile..self.Place..'.txt', httpService:JSONEncode(savedata))
end
...
mainapi.Categories.Main:CreateDropdown({
	...
	Function = function(val, mouse)
		if mouse then
			writefile('oofer-vape/profiles/gui.txt', val)
			shared.vapereload = true
			if shared.VapeDeveloper then
				loadstring(readfile('oofer-vape/loader.lua'), 'loader')()
			else
				loadstring(game:HttpGet('https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/'..readfile('oofer-vape/profiles/commit.txt')..'/loader.lua', true))()
			end
		end
	end
})
		...
		for i, v in mainapi.Modules do
			if v.Enabled or table.find(found, i) then
				...
				if interfacebar.Enabled then
					...
					holderline.Image = getcustomasset('oofer-vape/assets/rise/slice.png')
